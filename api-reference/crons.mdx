...
| `durationMs`     | integer or null | Elapsed execution time when the run has completed.             |
...
| `items[].durationMs`     | integer or null | Elapsed execution time when the run has completed.             |
...
| `responseStatus`            | integer or null | —                                                                                                                                                     |
| `error`                     | string or null  | —                                                                                                                                                     |
| `responseBody`              | object          | Bounded target response body retained for inspection. Secret-like JSON fields are redacted and provider-specific infrastructure terms are normalized. |
| `responseBodyTruncated`     | boolean         | Whether the retained response body was cut at the 32,768-byte limit.                                                                                  |
