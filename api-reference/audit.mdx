...
| `actor.kind`  | string         | One of: `user`, `api_key`, `agent_grant`.                                                                                                              |
| `actor.label` | string or null | The user's email or the API key's name, captured when the entry was written — entries outlive removed members and revoked keys. Null for agent grants. |
