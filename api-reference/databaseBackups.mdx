...
| `items[].purgedAt` | string or null | Set after the backing repository confirms physical deletion of this recovery point. Format: `date-time`. |
...
| `purgedAt` | string or null | Set after the backing repository confirms physical deletion of this recovery point. Format: `date-time`. |
...
| `200` | Backup deleted. |
| `404` | No such backup for this database. |
| `409` | The capture is still active, its physical deletion cannot yet be confirmed, or another recovery operation still depends on it. |
