...
| `region` | string | Where the app runs: a compute region id for always-on apps, "global" for scale-to-zero apps (they run on the global edge). Always-on projects can change regions with projects.update; the catalog lives at regions.list. |
...
| `runtimeClass` | string | No | Deprecated compatibility input. New projects use automatic scaling when omitted. One of: `scale-to-zero`, `always-on`. |
| `region` | string | No | Compute region id for always-on apps (see regions.list); defaults to the platform default region. Scale-to-zero apps run on the global edge and must not pick a region. |
...
| `422` | Invalid name, slug, runtime class, region, or repository access — including any region on a scale-to-zero project. A repository installed for another organization is never accepted. |
| `502` | Repository access could not be verified — try again. |
...
| `region` | string | No | Compute region id for an always-on project (see regions.list). Scale-to-zero projects run globally and cannot set a region. |
...
| `404` | No such project in this organization. |
| `422` | Invalid name, gitRepo, gitRef, region, or repository access — or no fields to update. |
| `502` | Repository access could not be verified — try again. |
...
```bash
curl "https://api.korve.dev/v1/orgs/$ORG_ID/repositories" \
  -H "Authorization: Bearer korve_..."
```
...
| Status | Description |
| ------ | ----------- |
| `200` | The organization's installable repositories. |
| `502` | The repository host could not be reached — try again. |

### Response body (200)

| Field | Type | Description |
| ----- | ---- | ----------- |
| `installed` | boolean | Whether a GitHub App installation backs this organization. |
| `installUrl` | string | Where to install the Korve GitHub App for this organization. |
| `repositories` | object[] | — |
| `repositories[].fullName` | string | Repository as "owner/repo". |
| `repositories[].private` | boolean | — |
