...
- Timestamps are RFC 3339 strings; durations of retention and expiry are
  stated per endpoint.
- Organization and project address parameters accept their id or slug where
  the operation documents that shared convention. Other resource parameters
  use the exact id or name stated on the operation.
- Operations are identified by stable ids (`projects.create`,
  `logs.query`) — the same ids appear in [MCP `search`](/agents/mcp-setup)
  results. MCP execution uses their declared method and path through
  `korve.request`.
