...
```text
POST /v1/agent-auth/device-authorizations
```
...
```bash
curl -X POST "https://api.korve.dev/v1/agent-auth/device-authorizations" \
  -H "Content-Type: application/json" \
  -d '{
  "clientId": "korve-cli",
  "audience": "cli",
...
| Status | Description                                                                                          |
| ------ | ---------------------------------------------------------------------------------------------------- |
| `201`  | Device authorization created. Show the user code, then poll the token endpoint with the device code. |
| `422`  | Invalid client, audience, scope, or role.                                                            |
| `429`  | Too many device authorization attempts from this network source.                                     |

### Response body (201)

| Field                     | Type    | Description                                                                                                                                   |
| ------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `deviceCode`              | string  | Client-held polling secret; never shown to the approving user.                                                                                |
| `userCode`                | string  | Human-entered pairing code in XXXX-XXXX form, from an unambiguous alphabet. Pattern: `^[BCDFGHJKLMNPQRSTVWXZ]{4}-[BCDFGHJKLMNPQRSTVWXZ]{4}$`. |
| `verificationUri`         | string  | Format: `uri`.                                                                                                                                |
| `verificationUriComplete` | string  | The verification page with the user code pre-filled. Format: `uri`.                                                                           |
| `expiresIn`               | integer | —                                                                                                                                             |
| `interval`                | integer | Minimum seconds between token polls.                                                                                                          |
...
```text
GET /v1/agent-auth/device-authorizations/{userCode}
```
...
| ---------- | ------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `userCode` | string | Yes      | Human-entered pairing code in XXXX-XXXX form, from an unambiguous alphabet. Pattern: `^[BCDFGHJKLMNPQRSTVWXZ]{4}-[BCDFGHJKLMNPQRSTVWXZ]{4}$`. |

### Example

```bash
curl "https://api.korve.dev/v1/agent-auth/device-authorizations/$USER_CODE" \
  -b "$SESSION"
```
...
| Status | Description                                                  |
| ------ | ------------------------------------------------------------ |
| `200`  | Safe approval details; the device code is never returned.    |
| `404`  | Unknown, expired, approved, denied, or consumed device code. |
...
```text
POST /v1/orgs/{orgId}/agent-grants/device-authorizations/{userCode}/approve
```
...
```bash
curl -X POST "https://api.korve.dev/v1/orgs/$ORG_ID/agent-grants/device-authorizations/$USER_CODE/approve" \
  -b "$SESSION" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
| Status | Description                                                           |
| ------ | --------------------------------------------------------------------- |
| `200`  | Device authorization approved; the device completes on its next poll. |
| `404`  | Unknown, expired, approved, denied, or consumed device code.          |
...
```text
POST /v1/agent-auth/device-authorizations/{userCode}/deny
```
...
```bash
curl -X POST "https://api.korve.dev/v1/agent-auth/device-authorizations/$USER_CODE/deny" \
  -b "$SESSION" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
| Status | Description                                                  |
| ------ | ------------------------------------------------------------ |
| `200`  | Device authorization denied.                                 |
| `404`  | Unknown, expired, approved, denied, or consumed device code. |
...
| Field          | Type   | Required | Description                                                   |
| -------------- | ------ | -------- | ------------------------------------------------------------- |
| `grantType`    | string | Yes      | One of: `authorization_code`, `device_code`, `refresh_token`. |
| `clientId`     | string | Yes      | One of: `korve-cli`, `korve-mcp`.                             |
| `redirectUri`  | string | No       | Format: `uri`.                                                |
| `code`         | string | No       | —                                                             |
| `codeVerifier` | string | No       | —                                                             |
| `deviceCode`   | string | No       | The device authorization's client-held polling secret.        |
...
| `400`  | Invalid, expired, consumed, replayed, or mis-bound grant. Device polling answers with the error codes AUTHORIZATION_PENDING, SLOW_DOWN, ACCESS_DENIED, or EXPIRED_TOKEN. |
