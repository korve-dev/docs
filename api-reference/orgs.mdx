...
Get the organization's email notification preferences: which platform events email the owners and admins. Webhook deliveries are unaffected by these toggles.

```text
GET /v1/orgs/{orgId}/notification-prefs
```
...
```bash
curl "https://api.korve.dev/v1/orgs/$ORG_ID/notification-prefs" \
  -b "$SESSION"
```
...
| Status | Description                     |
| ------ | ------------------------------- |
| `200`  | The email notification toggles. |

### Response body (200)

| Field            | Type    | Description                                                         |
| ---------------- | ------- | ------------------------------------------------------------------- |
| `deployFailed`   | boolean | Email the organization's owners and admins when a deployment fails. |
| `budgetAlert`    | boolean | Email when a project passes its budget alert threshold.             |
| `budgetExceeded` | boolean | Email when a project reaches its monthly budget cap.                |

## orgs.setNotificationPrefs

Update the organization's email notification preferences. Only the toggles provided change; webhook deliveries are unaffected.

```text
PATCH /v1/orgs/{orgId}/notification-prefs
```
...
| Field            | Type    | Required | Description                                                         |
| ---------------- | ------- | -------- | ------------------------------------------------------------------- |
| `deployFailed`   | boolean | No       | Email the organization's owners and admins when a deployment fails. |
| `budgetAlert`    | boolean | No       | Email when a project passes its budget alert threshold.             |
| `budgetExceeded` | boolean | No       | Email when a project reaches its monthly budget cap.                |

### Example

```bash
curl -X PATCH "https://api.korve.dev/v1/orgs/$ORG_ID/notification-prefs" \
  -b "$SESSION" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
| Status | Description                                  |
| ------ | -------------------------------------------- |
| `200`  | The updated toggles.                         |
| `422`  | No toggles provided, or a non-boolean value. |
