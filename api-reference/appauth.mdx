...
| `github.clientId`         | string or null  | —                                                                                       |
| `github.secretConfigured` | boolean         | —                                                                                       |
| `github.secretVersion`    | integer or null | —                                                                                       |
| `github.rotatedAt`        | string or null  | Format: `date-time`.                                                                    |
| `callbackUrl`             | string          | Exact Google OAuth callback URL to register for this project.                           |
| `githubCallbackUrl`       | string          | Exact GitHub OAuth callback URL to register for this project.                           |
...
Configure GitHub OAuth with the customer's client id and a secret encrypted at ingestion.

```text
PUT /v1/orgs/{orgId}/projects/{projectId}/auth/providers/github
```
...
```bash
curl -X PUT "https://api.korve.dev/v1/orgs/$ORG_ID/projects/$PROJECT_ID/auth/providers/github" \
  -b "$SESSION" \
  -H "Content-Type: application/json" \
  -d '{
  "clientId": "example",
...
Replace the encrypted GitHub OAuth secret and increment its rotation version.

```text
POST /v1/orgs/{orgId}/projects/{projectId}/auth/providers/github/rotate
```
...
```bash
curl -X POST "https://api.korve.dev/v1/orgs/$ORG_ID/projects/$PROJECT_ID/auth/providers/github/rotate" \
  -b "$SESSION" \
  -H "Content-Type: application/json" \
  -d '{
  "clientSecret": "example"
...
| `identities`                 | object[]       | —                                                       |
| `identities[].provider`      | string         | One of: `google`, `github`, `credential`, `magic_link`. |
...
Create a single-use provider-bound state and PKCE challenge for an exact allowed redirect URL.

```text
POST /v1/apps/{projectId}/auth/oauth/github/start
```
...
```bash
curl -X POST "https://api.korve.dev/v1/apps/$PROJECT_ID/auth/oauth/github/start" \
  -H "Content-Type: application/json" \
  -d '{
  "redirectUrl": "https://app.acme.com/auth/callback"
}'
...
Validate the provider-bound one-time OAuth state and GitHub response, then redirect with a short-lived exchange code.

```text
GET /v1/apps/{projectId}/auth/oauth/github/callback
```
...
```bash
curl "https://api.korve.dev/v1/apps/$PROJECT_ID/auth/oauth/github/callback?state=example"
```
