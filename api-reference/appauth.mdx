...
Send a single-use passwordless sign-in link. The response never reveals whether an account existed.
...
Consume a single-use email link and return a new application session.
...
Send a single-use password reset link. The response never reveals whether an account existed.
