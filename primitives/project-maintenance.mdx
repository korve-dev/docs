...
The current launch scope retains failed-deployment findings in `reliability`, with severity, stable
resource identity, recommendation, and first/last observed times. Evidence is immutable, redacted,
and records whether it came from a metric, log, configuration, query, or verification step. Other
maintenance categories are not accepted by the API or manifest yet.
