...
Enable collection with the CLI. Korve always allows the managed project origin and active custom
domains. Use `--origin` only for another exact HTTPS origin:

```bash
korve speed-insights show --project storefront
korve speed-insights set --state enabled --sample-rate 1 \
