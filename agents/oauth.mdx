...
automatically. On macOS, the CLI first stores the access and refresh tokens in Keychain under the
`dev.korve.cli.oauth` service, keyed by API URL. If Keychain is unavailable—or when
running on Linux, or when `KORVE_CONFIG_DIR` selects an isolated configuration directory—the tokens
use the separate `${KORVE_CONFIG_DIR:-~/.config/korve}/oauth-credentials.toml` fallback with `0600`
permissions. Organization, grant, and
