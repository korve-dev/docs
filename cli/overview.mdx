...
4. `${KORVE_CONFIG_DIR:-~/.config/korve}/oauth-credentials.toml` for OAuth tokens,
   with `0600` on Unix; `~/.config/korve/config.toml` stores login metadata or a legacy key
